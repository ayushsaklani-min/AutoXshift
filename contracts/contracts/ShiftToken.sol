...
/**
 * @title ShiftToken
 * @dev ERC20 token for SideShift integration
 * @notice This is a test token for Polygon Amoy testnet
 */
contract ShiftToken is ERC20, Ownable, Pausable {
  ...
  constructor() ERC20("Shift Token", "SHIFT") Ownable(msg.sender) {
    ...
    function mint(address to, uint256 amount) external {
        require(minters[msg.sender], "ShiftToken: caller is not a minter");
        require(totalSupply() + amount <= MAX_SUPPLY, "ShiftToken: max supply exceeded");
        _mint(to, amount);
    }
